function first_two_fix_salience_comparison(control_trials, inactivation_trials, salmethod)
% FIRST_TWO_FIX_SALIENCE_COMPARISON compares global salience after 
%	splitting trials by center of mass of first two fixations
% Inputs
...
% July 5, 2017
	...
	for i = 1:ni
		...
		first_two_com = get_first_n_com(trial,2);

		if first_two_com < (1920/2)
...
function com = get_first_n_com(trial,n)

	if n > trial.n_fixations
		warnstr = sprintf('Too many fixations requested. Only using %d for this trial',...
			trial.n_fixations);
		warning(warnstr);
		xs = zeros(trial.n_fixations,1);
	else
		xs = zeros(n,1);
	end
	...
	for i = 1:n
		if i < trial.n_fixations
			xs(i) = trial.fixations{i}.cx;
		end
	end
