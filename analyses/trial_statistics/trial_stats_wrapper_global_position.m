function trial_stats_wrapper_global_position(datasets, mode, order)
% TRIAL_STATS_WRAPPER left/right mean left/right globally
% Inputs
...
%	None
	...
	case 'aggregate'
		...
		control_left_stats = trial_stats(control_trials, 'global_left',order,...
			'control-left'); 
		control_right_stats = trial_stats(control_trials, 'global_right',order,...
			'control-right'); 

		inactivation_left_stats = trial_stats(inactivation_trials,...
			'global_left',order,	'inactivation-left'); 
		inactivation_right_stats = trial_stats(inactivation_trials,...
			'global_right',order, 'inactivation-right'); 
	...
	case 'aggregate_summaries'
		...
		control_left_stats = aggregate_stat_summaries(datasets,...
			'control', 'global_left', order, 'control-left');
		control_right_stats = aggregate_stat_summaries(datasets,...
			'control', 'global_right', order, 'control-right');

		inactivation_left_stats = aggregate_stat_summaries(datasets,...
			'inactivation', 'global_left', order, 'inactivation-left');
		inactivation_right_stats = aggregate_stat_summaries(datasets,...
			'inactivation', 'global_right', order, 'inactivation-right');
		...
		for i = 1:n_d
			...
			if (size(inactivation_trials,1) > 0)
				...
				control_left_stats = trial_stats(...
					control_trials,'global_left',order,...
					'control-left');
				control_right_stats = trial_stats(...
					control_trials,'global_right',order,...
					'control-right');
				inactivation_left_stats = trial_stats(...
					inactivation_trials,'global_left',order,...
					'inactivation-left');
				inactivation_right_stats = trial_stats(...
					inactivation_trials,'global_right',order,...
					'inactivation-right');
